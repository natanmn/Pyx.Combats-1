	...
	if monsterActor then
		...
		if not selfPlayer.IsActionPending then
			...
			if actorPosition.Distance3DFromMe > monsterActor.BodySize + 150 then
				...
				and actorPosition.Distance3DFromMe >= monsterActor.BodySize + 300 and selfPlayer.ManaPercent >= 15 
				and not selfPlayer:IsSkillOnCooldown(CELESTIAL_SPEAR) then
            ...
            and monsterCount >= 3 and selfPlayer.BlackRage == 100 and selfPlayer.ManaPercent >= 30 then
                ...
                if JUDGEMENT_OF_LIGHT == 772 then
                    selfPlayer:DoActionAtPosition("BT_Party_Skill_LightJudgment_Start_1LV", actorPosition, 3000)
                elseif JUDGEMENT_OF_LIGHT == 773 then
                    selfPlayer:DoActionAtPosition("BT_Party_Skill_LightJudgment_Start_2LV", actorPosition, 3000)
                else
                    selfPlayer:DoActionAtPosition("BT_Party_Skill_LightJudgment_Start_3LV", actorPosition, 3000)
                end
                return
            end
			...
			and actorPosition.Distance3DFromMe <= monsterActor.BodySize + 200 and selfPlayer.ManaPercent >= 20 and selfPlayer:IsSkillOnCooldown(SHIELD_THROW)
			and selfPlayer:IsSkillOnCooldown(CELESTIAL_SPEAR) then
