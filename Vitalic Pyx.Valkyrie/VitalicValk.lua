...
-------------------------------------------------------------------------------------------------------------------
	...
	if monsterActor then
		...
		if not selfPlayer.IsActionPending then
			...
			if JUDGEMENT_OF_LIGHT ~= 0 and selfPlayer:IsSkillOnCooldown(SHIELD_THROW) and selfPlayer:IsSkillOnCooldown(CELESTIAL_SPEAR) and not selfPlayer:IsSkillOnCooldown(JUDGEMENT_OF_LIGHT) and actorPosition.Distance3DFromMe <= monsterActor.BodySize + 500
