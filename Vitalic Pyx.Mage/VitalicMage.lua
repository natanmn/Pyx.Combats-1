...
function VitalicMage:Roaming()
	...
	self:CheckFireballStuck("Roaming")
...
------------------------------------------------------------------------------------------------------
	...
	if monsterActor then
		...
		if actorPosition.Distance3DFromMe > monsterActor.BodySize + 1300 or not monsterActor.IsLineOfSight then
			Navigator.MoveTo(actorPosition)
		else
			Navigator.Stop()

		-- Mob too close? Cast Freeze
		if FREEZE ~= 0 and not selfPlayer:IsSkillOnCooldown(FREEZE) and selfPlayer.ManaPercent >= 15 and actorPosition.Distance3DFromMe <= monsterActor.BodySize + 300 then
			selfPlayer:SetActionStateAtPosition(ACTION_FLAG_MOVE_BACKWARD | ACTION_FLAG_SPECIAL_ACTION_2, actorPosition, 900)
			return
		end

		-- Cast Fireball
		if FIREBALL ~= 0 and not selfPlayer:IsSkillOnCooldown(FIREBALL) and not selfPlayer:IsSkillOnCooldown(FIREBALL_EXPLOSION) and selfPlayer.ManaPercent > 15
		then
			selfPlayer:SetActionStateAtPosition(ACTION_FLAG_MOVE_BACKWARD | ACTION_FLAG_MAIN_ATTACK, actorPosition, 900)
			return
		end
		...
		-- Mana Low?
		if MANA_ABSORPTION ~= 0 and selfPlayer.ManaPercent < 20 and not selfPlayer:IsSkillOnCooldown(MANA_ABSORPTION)
		then
		...
		-- Cast Lightning
		if LIGHTNING ~= 0 and selfPlayer.ManaPercent >= 15 and (not selfPlayer:IsSkillOnCooldown(LIGHTNING) and not selfPlayer:IsSkillOnCooldown(RESIDUAL_LIGHTNING)
		and not string.match(selfPlayer.CurrentActionName, "Fireball")) then
		...
		if LIGHTNING_CHAIN ~= 0 and selfPlayer:IsSkillOnCooldown(FIREBALL_EXPLOSION) and selfPlayer:IsSkillOnCooldown(LIGHTNING)
		...
		and string.match(selfPlayer.CurrentActionName, "Chain_Lightning") then
